...
const overflowInlined = () => {
	return typeof $('body').attr('style') === 'undefined'
	|| $('body').attr('style').length < 1
	|| $('body').attr('style').toLowerCase().replace(/\s/g,'').indexOf('overflow:auto!important;') < 0;
}

const onBodyMutation = (mutationList, observer) => {
